...
local Window = Fluent:CreateWindow({
	Title = "WusElaina Bá Đạo Vcl",
	SubTitle = "Script Bí Mật",
	TabWidth = 160,
					...
					["embeds"] = {
						{
							...
							["footer"]= {
								["text"]  = "WusElaina Thông Báo Fisch",
								["icon_url"] = "https://files.catbox.moe/67zgld.jpg"
							}
						}
					}
				})
...
Seller = Tap.General:AddSection('Seller') do
	Toggle(Seller, "Tự động bán cá","", "Bán cá")
...
InfoFish = Tap.General:AddSection('Thông tin cá') do
	...
	InfoFavouriteBait = InfoFish:AddParagraph({        
		Title = "Mồi ưa thích : N/A"
	})
	
	Dropdown(InfoFish, "Chọn Cá", "", FishList, false, "Chọn thông tin cá")
	InfoFish:AddButton({
		Title = "Nhận thông tin Cá trong tay",
		Callback = function()
			for i,v in pairs(LocalPlayer.Character:GetChildren()) do
				if table.find(FishList, v.Name) then
					Config['Chọn thông tin cá'] = v.Name
					InfoPrice:SetTitle("Giá : "..FISHDATA[Config['Chọn thông tin cá']].Price)
					InfoRarity:SetTitle("Độ hiếm : "..FISHDATA[Config['Chọn thông tin cá']].Rarity)
					InfoChance:SetTitle("Cơ hội : "..FISHDATA[Config['Chọn thông tin cá']].Chance.." %")
					InfoFavouriteBait:SetTitle("Mồi ưa thích : "..FISHDATA[Config['Chọn thông tin cá']].FavouriteBait)
					return
				end
			end
			Notify("Xin hãy trang bị cá")
		end
	})
	InfoFish:AddButton({
		Title = "Nhận thông tin Cá",
		Callback = function()
			if Config['Chọn thông tin cá'] == "" then
				Notify("Xin vui lòng chọn cá")
				return
			end
